...
export class StreamingDetector {
    ...
    updateConfiguration(): void {
        ...
        // Users can extend this list via `privacyGuard.recordingProcesses`.
        this.streamingProcesses = config.get('recordingProcesses', [
            'obs',
            'obs64',
            'streamlabs obs',
            'streamlabs desktop',
    ...
    startDetection(callback?: (isStreaming: boolean) => void): void {
        this.detectionInterval = setInterval(async () => {
            try {
                ...
                console.error('Error detecting recording software:', error);
            }
        ...
        this.detectStreamingSoftware().then(isStreaming => {
            ...
            console.error('Error in initial recording detection:', error);
        });
    }
